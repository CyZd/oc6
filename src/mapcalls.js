    ...
    if(requestAlreadyExist){
        ...
        ajaxGet(url,function(response){
            ...
            for(keys of results.records){
                newPoint=createPoint(keys.fields,firstParam,secondParam,thirdParam,fourthParam);
...
function pushToLayer(globalResults,url,firstParam,secondParam,thirdParam,fourthParam){
    result=findByName(globalResults,'name',url);
    for(point of result.points){
        globalPoints.push(L.marker(point[firstParam],{uniqueID: url, visible:true}).bindPopup(point[secondParam]+"<br>"+point[thirdParam]+"<br>"+point[fourthParam]+" mètres de vous"));
    }
